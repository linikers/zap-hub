...
Installation
-----------
Pick ONE method. On Linux, the shell script is the easiest.

```bash
# Shell script (installs to /usr/local/bin, works on Linux + macOS)
curl -fsSL https://raw.githubusercontent.com/xdevplatform/xurl/main/install.sh | bash

# Homebrew (macOS)
brew install --cask xdevplatform/tap/xurl

# npm (can fail on Windows — see below)
...
**Windows note:** `npm install -g @xdevplatform/xurl` may fail with `MODULE_NOT_FOUND` at runtime even after successful install, especially after system instability — the `cli.js` entry point can go missing from `C:\Program Files\nodejs\node_modules\@xdevplatform\xurl\`. Fix: `npm uninstall -g @xdevplatform/xurl && npm install -g @xdevplatform/xurl`. For a more reliable approach, download `xurl_Windows_x86_64.zip` from [releases](https://github.com/xdevplatform/xurl/releases), extract manually, and add to PATH.
...
> **Headless server / Cloudflare workaround:** If you are on a server without a browser (headless Linux, SSH, Docker) or Cloudflare blocks the browser-based OAuth flow, see `references/headless-setup.md` for manual token setup.
...
| `CreditsDepleted` | $0 balance on X API | Buy credits (min $5) in Developer Console → Billing. Affects ANY write operation (post, media upload, profile update) via the API. Read operations (whoami, search, timeline) may still work on free tier. Workaround verificado: com a conta logada no preview pane do desktop (desktop_preview + drive_preview), postar e responder FUNCIONA — digitar no composer (data-testid=tweetTextarea_0) e clicar em data-testid=tweetButton publica de verdade (confirmado pelo toast "Seu post foi enviado" + post no perfil). Leitura também: a página do perfil/busca no browser logado mostra textos e métricas sem gastar crédito. Alternative: use Zernio (zernio.com) which handles X API pass-through billing at $0.015/post — but still needs a card on file. |
| `media processing failed` on image upload | Default category is `amplify_video` | Add `--category tweet_image --media-type image/png` |
| Token expirado (`expiration_time` no passado) e sem browser | Não dá pra rodar `xurl auth oauth2` em servidor headless | Renovar via refresh_token grant headless — ver `references/token-refresh-headless.md` (funciona, HTTP 200) |
...
| Browser OAuth fails on headless server | Cloudflare blocks headless browser | See `references/headless-setup.md` for alternatives: (a) do OAuth on desktop + copy ~/.xurl, or (b) try Camoufox anti-detection browser + xdg-open URL capture trick |
| Cloudflare redirect/captcha loop on headless Chromium | Automated browser detection | Try Camoufox (Firefox anti-detection) se for servidor headless — ele bypassa Cloudflare e carrega o X. Mas o formulário React pode não responder à automação via REST API. Melhor: fazer OAuth no desktop |
| Camoufox carrega login do X mas botão Next não aparece | Formulário React do X não expõe botão de submit | Tentar Google SSO (Solução C no references/headless-setup.md) — usa o iframe do Google em vez do formulário de email. Se Google também não funcionar, cair pra Solução A (desktop + copiar ~/.xurl) |
...
- **Headless OAuth:** Para servidores sem display, ver `references/headless-oauth.md` — guia com Camoufox e alternativas.
