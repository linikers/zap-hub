...
§
Taiff Connect: api.taiff-connect.com.br. NestJS+PostgreSQL+RabbitMQ. Deploy EKS (Helm). SendGrid trial fim 03/ago/2026. DOMÍNIO: taiff.com.br DMARC p=reject → noreply@taiff-connect.com.br. PVC emptyDir (gp2 Pending). Front campos inglês, backend PT (PR #104). Forgot-password email+SMS 6 dígitos. Upload foto: campo "file". Test user: eng@ik1.com.br/eng@@2027. NUNCA alterar rotas sem análise.
§
...
§
Pollinations.ai: geração gratuita de imagens por URL, sem API key. https://image.pollinations.ai/prompt/...?width=X&height=Y&nologo=true. Bom para fundos, NUNCA para veículos específicos. Banner com foto real: rembg (u2net) extrai fundo → PIL composição. Skill: banner-ecommerce-automotivo.