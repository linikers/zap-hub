...
async function startSocket() {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (qr) {
      ...
      console.log('\nWaiting for scan...\n');
      // Save QR data to file for image generation
      try {
        writeFileSync('/tmp/whatsapp-qr-data.txt', qr);
      } catch(e) {}
    }
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (!msg.key.fromMe) {
        // Self-chat mode also processes incoming messages from allowed users
        // (e.g., Alcides can message the user and Hermes responds).
