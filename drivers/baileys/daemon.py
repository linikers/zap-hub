...
LOG_FILE = os.environ.get("NFE_LOG_FILE", "daemon.log")
...
log = logging.getLogger("zap-daemon")

# Bot module: procura por bot.py no diretorio de execucao
BOT_MODULE = os.environ.get("BOT_MODULE", "bot")  # nome do modulo (sem .py)
sys.path.insert(0, os.getcwd())

try:
    bot_mod = __import__(BOT_MODULE)
    detectar_intencao_nfe = getattr(bot_mod, "detectar_intencao_nfe", None)
    handle_nfe_message = getattr(bot_mod, "handle_nfe_message", None)
    responder_fora_do_escopo = getattr(bot_mod, "responder_fora_do_escopo", None)
    responder_fora_do_horario = getattr(bot_mod, "responder_fora_do_horario", None)
    dentro_do_horario = getattr(bot_mod, "dentro_do_horario", None)
    log.info("Bot module loaded: %s.py", BOT_MODULE)
except ImportError as e:
    log.error("Nao foi possivel carregar modulo do bot (%s.py): %s", BOT_MODULE, e)
    log.error("Execute o daemon do diretorio do bot (ex: bots/nfe/)")
    sys.exit(1)
